...
from data import credentials # External module with credentials data
...
"""
Here we use an external module to read the user credentials to login with 5 different users
For login we want only 1 time per user, hence, adding that code in on_start method and define instance attributes
"""
...
class UserBehaviour(SequentialTaskSet):
    ...
    def on_start(self):
        ...
        if(len(credentials.UserName) > 0):
            self.userName, self.Password = credentials.UserName.pop()
            print(self.userName, self.Password)
            credentials.UserName.insert(0, (self.userName, self.Password)) # just for testing purposes to avoid empty list 
            print(credentials.UserName)
        else:
            print("User List is empty")

        # Use user credentials in payload
